...
import { Sparkles, Send, Loader2, AlertCircle, ArrowLeft, ChevronRight, LayoutGrid, Type, Image as ImageIcon } from "lucide-react";
import { 
...
export default function App() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // 1. Generate text content for selected platforms
      const platformContents = await generateSocialContent(idea, tone, selectedPlatforms);
      
      // Initialize posts with text and loading state for images
      ...
      const imagePromises = platformContents.map(async (pc, index) => {
        try {
          // Use the global aspect ratio selected by the user
          ...
          setPosts(prev => {
            ...
            newPosts[index] = { 
              ...
              error: isPermissionError 
                ? "Permission denied. This model requires a valid paid API key. Please ensure billing is enabled: https://ai.google.dev/gemini-api/docs/billing" 
  ...
  const handleRegeneratePost = async (index: number) => {
    ...
    try {
      const newContent = await regenerateSinglePost(idea, tone, post.platform);
      
      // Update text and image prompt
      setPosts(prev => {
        const newPosts = [...prev];
        newPosts[index] = { 
          ...newPosts[index], 
          ...
          imageUrl: null, // Reset image as prompt changed
          loading: true 
        };
        return newPosts;
      });
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-primary rounded-lg flex items-center justify-center text-white font-bold text-sm shadow-lg shadow-primary/20">S</div>
              <span className="font-bold tracking-tighter hidden sm:block dark:text-white">SocialGen AI</span>
            </div>
          </div>
      ...
      <footer className="max-w-7xl mx-auto px-6 py-16 border-t border-gray-100 mt-20">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs font-bold uppercase tracking-widest text-gray-400">SocialGen AI Engine v2.0</span>
          </div>
          <div className="flex items-center gap-12">
            <div className="flex flex-col items-end">
              <span className="text-[10px] font-bold uppercase tracking-widest text-gray-300">Model</span>
              <span className="text-xs font-bold text-gray-900">Gemini 3.1 Pro</span>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-[10px] font-bold uppercase tracking-widest text-gray-300">Visuals</span>
              <span className="text-xs font-bold text-gray-900">Gemini 3 Pro Image</span>
            </div>
          </div>
        </div>
      </footer>
